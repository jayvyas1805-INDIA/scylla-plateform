...
function DisciplineModal({ initialData, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
      <div className="bg-admin-surface-raised border border-white/10 rounded-lg p-4 w-full max-w-md">
        <h3 className="text-admin-accent text-sm font-semibold mb-3">
          {initialData ? "Edit Discipline" : "Add Discipline"}
        </h3>
        <div className="space-y-2">
          <input
            className="w-full bg-admin-surface-raised text-white text-xs p-2 rounded border border-white/10"
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              className="bg-admin-surface-raised text-white text-xs p-2 rounded border border-white/10"
        ...
        <div className="flex justify-end gap-2 mt-3">
          ...
          <button
            ...
            className="text-black bg-admin-accent text-xs px-3 py-1 rounded hover:bg-admin-accent"
          >
            Save
          </button>
        </div>
...
export default function MotorsportDisciplinesSection() {
  ...
  return (
    <section className="bg-admin-surface border border-white/10 rounded-xl shadow">
      {/* Header */}
      <div className="flex justify-between items-center px-6 py-4 border-b border-white/10">
        <h2 className="text-sm font-semibold text-admin-accent">Motorsport Disciplines</h2>
        <button
          onClick={() => {
            setEditing(null);
            setModalOpen(true);
          }}
          className="flex items-center gap-1 text-xs text-admin-accent hover:text-admin-accent"
        >
          <Plus size={14} />
          Add Discipline
        </button>
  ...
  <div className="flex gap-2">
    <select
      ...
      className="bg-admin-surface-raised text-white text-xs p-1 rounded border border-white/10"
    >
    ...
    >
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1">
                    {item.linkedClasses.map((cls, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-0.5 rounded text-[11px] bg-admin-accent/10 text-admin-accent border border-admin-accent/20"
                      >
                        {cls}
                      </span>
                    ))}
                ...
                <td className="px-4 py-3">
                  <div className="flex justify-center gap-2">
                    <button
                      ...
                      className="text-admin-accent hover:bg-admin-accent/10 p-1.5 rounded"
                    >
                      <Edit size={14} />
                    </button>
      ...
      <div className="border-t border-white/10 mt-4 px-6 py-4">
        <h3 className="text-xs font-semibold text-admin-accent mb-3">
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex justify-between items-center bg-admin-surface-raised border border-white/10 rounded px-4 py-2"
            >
