...
const CategoryTabs = ({ activeTab, setActiveTab }) => {
  return (
    <div className="bg-admin-surface border border-white/10 rounded-xl shadow-[0_0_25px_rgba(0,0,0,0.6)] p-4 flex flex-wrap gap-2 mb-6">
      {tabs.map((t) => {
        const Icon = t.icon;
        return (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 text-sm rounded transition
              ${
                activeTab === t.id
                  ? "bg-admin-accent text-black font-semibold"
