...
export default function ModerationReportsPanel() {
  ...
  return (
    <aside className="w-full bg-admin-surface border border-admin-border rounded-xl p-4 sm:p-5 space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full h-24 sm:h-28 text-xs sm:text-sm bg-admin-bg border border-admin-border rounded p-2 resize-none"
        />

        <button
          onClick={() => toast.success("Notes saved")}
          className="w-full mt-3 bg-admin-accent-dark hover:bg-admin-accent-dark text-xs sm:text-sm py-2 rounded transition"
        >
          Save Notes
        </button>
      </div>
...
const Report = ({ id, text, status }) => {
  ...
  return (
    <div className="border border-admin-border rounded-lg p-3 text-xs sm:text-sm">
...
const ActionBtn = ({ icon, label, color, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center justify-center gap-2 py-2 text-xs sm:text-sm rounded border border-admin-border transition hover:opacity-90 ${colors[color]}`}
    >
      {icon} {label}
    </button>
  );
