...
function Badge({ children, tone }) {
  const map = {
    blue: "bg-admin-accent/15 text-admin-accent ring-admin-accent/30",
...
export default function ReviewModerationPanel() {
 ...
 const toggleVisibility = (id) => {
  ...
  return (
    <div className="p-6 bg-admin-bg rounded-2xl shadow-2xl">
            ...
            {TYPES.map((t) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-sm ring-1 transition ${
                  type === t
                    ? "bg-admin-accent-dark/20 text-admin-accent ring-admin-accent/40"
      ...
      <div className="hidden lg:block overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-admin-surface text-slate-400">
            <tr>
