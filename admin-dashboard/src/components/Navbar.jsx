...
export default function Navbar({ toggleSidebar }) {
 ...
 useEffect(() => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-[60px] bg-admin-surface flex items-center justify-between px-6 text-white z-50 shadow-md">
      {/* LEFT */}
      <div className="flex items-center gap-4">
        {/* Sidebar Toggle Button */}
        <button
          onClick={toggleSidebar}
          className="p-2 rounded hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-admin-accent"
      ...
      <div className="flex items-center gap-3 relative">
        ...
        {open && (
          <div
            ref={menuRef}
            className="absolute top-[60px] right-6 w-[300px] bg-admin-surface border border-gray-800 rounded-2xl shadow-2xl z-50"
          >
            {/* Dropdown content */}
